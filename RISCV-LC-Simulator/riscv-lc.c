...
void cycle_memory() {
    ...
    if (mio_en) {
        ...
        if (W) {
             ...
             * Lab3-2 assignment
             */
            int operation = datasize_mux(get_DATASIZE(CURRENT_LATCHES.MICROINSTRUCTION),mask_val(CURRENT_LATCHES.IR, 14, 12),0);
            switch (operation) {
              case -1:
...
void eval_bus_drivers() {
    ...
    /* input of GateMAR */
    // value_of_GateMAR = ?;
	value_of_GateMAR = mar_mux(get_MARMUX(CURRENT_LATCHES.MICROINSTRUCTION),
                               value_of_MARMUX,
...
void latch_datapath_values() {
    ...
    if (get_LD_PC(CURRENT_LATCHES.MICROINSTRUCTION)) {
         ...
         *  Lab3-2 assignment
         */
        NEXT_LATCHES.PC = pc_mux(get_PCMUX(CURRENT_LATCHES.MICROINSTRUCTION),CURRENT_LATCHES.PC + 4, CURRENT_LATCHES.PC -4 + BUS);
